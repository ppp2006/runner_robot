...
void CImageProcessClass::testVedio(string sPath)
{
#if TEST
      //save image
      system("rm -rf ./Result");
      ...
      system("rm -rf ./Orignal");
      system("mkdir -p ./Orignal");
#endif

       //step1: get camera calibrate data
       double dCaliX = 0;
       double dCaliY = 0;
	...
	VideoCapture capture(sPath.c_str());// runway061301.mp4 runway062202.avi 0530 061301拐弯

        capture.set(CV_CAP_PROP_CONVERT_RGB, 1);
	//检测是否正常打开:成功打开时，isOpened返回ture
	if(!capture.isOpened())
	{
	    cout<<"vedio fail to open!"<<endl;
            return;
	}
	//获取整个帧数
	...
	szChessBoard.width = 9;
       ...
       // string ss = "distortion.txt";
        const char* pFile = "distortion.txt";
        proc.distortionData(pFile, intrinsics, distortion_coeff, true);
	...
	while(!IsStop)
	{
	  ...
	  //Sleep(100);
          ...
          //setp6: get detect result(distance\angle)
           double t1 = (double)getTickCount();
	   //nEdgeNum = runwayFunThree(frame, nIndex, rect, dst, ptStart, ptEnd, dAngle);//	
	   nEdgeNum = runwayFunOne(frame, nIndex, rect, dst, ptStart, ptEnd, dAngle);

 #if TEST  
          sprintf(buffer, "./Result/%d.bmp", nIndex);
          imwrite(buffer, dst);
#endif
...
#endif
           getAngleDiff(dAngle, dAngleStart, dAngleEnd, isFirstAngle, dAngleDiff); //angle: left top right
#if TEST
           cout<<"first="<<isFirstAngle<<","<<"start angle="<<dAngleStart<<","<<"end angle="<<dAngleEnd<<endl;
	   ...
	   //当时间结束前没有按键按下时，返回值为-1；否则返回按键
           ...
           sprintf(buffer, "nIndex=%d, start=(%d,%d), end=(%d,%d), angle=%.4f, Dis=%.4f, angle diff=%.4f, %d",
                  nIndex, ptStart.x, ptStart.y, ptEnd.x, ptEnd.y, dAngle, dDis, dAngleDiff, nEdgeNum);
           fun.WriteTxt("Result/result.txt", buffer);
#endif
...
void CImageProcessClass::testCamera(void)
{  
#if TEST
      //save image
      system("rm -rf ./Result");
...
#if 0
        ////////calibrate
        cv::line(frame,cvPoint(0, frame.rows/3),cvPoint(frame.cols-1, frame.rows/3),cv::Scalar(0),2); // x 1/3
        cv::line(frame,cvPoint(0, 2*frame.rows/3),cvPoint(frame.cols-1, 2*frame.rows/3),cv::Scalar(0),2); // x 1/3

        cv::line(frame,cvPoint(frame.cols/2, frame.rows/3),cvPoint(frame.cols/2, 2*frame.rows/3),cv::Scalar(0),2); // x 1/3
#endif
   ///////////////
#if TEST
	  imshow("Extracted frame", frame);
...
void CImageProcessClass::getAngleDiff(const double dAngle, double &dAngleStart, double &dAngleEnd, bool &isFirst, double &_dAngle)
{
   ...
   double dAngleStartTemp = dAngleStart;
   double dAngleEndTemp = dAngleEnd;
  
   {
      if (dAngleStartTemp < 0)
      {
          dAngleStartTemp = fabs(dAngleStartTemp);
      }
      else
      {
          dAngleStartTemp = 180 - dAngleStartTemp; 
      }

      if (dAngleEndTemp < 0)
      {
          dAngleEndTemp = fabs(dAngleEndTemp);
      }
      else
      {
          dAngleEndTemp = 180 - dAngleEndTemp; 
      }
      _dAngle = dAngleEndTemp - dAngleStartTemp;
   }
...
void CImageProcessClass::getCaliData(const char* pFile, double &_dCaliX, double &_dCaliY)
{
      string sFilePath;
      char sValue[256];
      fstream out;
   /*   char filename[256];  ///////////////////
      ...
      cout<<filename<<endl;*/
      out.open(pFile, ios::in); //sFilePath
...
{	
	...
	if (nNum >=2) //边缘>2个边缘，取距离图片x轴最近的2个边
	{		
