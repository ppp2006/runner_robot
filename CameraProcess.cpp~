...
void CCameraProcess::distortionData(const char* file, CvMat* intrinsics, CvMat* distortion_coeff, bool isRead)
{
	CCommonFunClass fun;
	...
	if (!isRead)
	{
		if (file)//!sFilePath.empty())
	        {
		    //system("rm );//DeleteFile(sFilePath.c_str());
                    remove(file);//sFilePath.c_str());
	        }
		for (int row=0; row<nH1; row++)
		{
			for (int col=0; col<nW1; col++)
			{
				dValue = intrinsics->data.fl[row * intrinsics->cols + col];
				sprintf(buffer, "%.4f", dValue);
				fun.WriteTxt(file, buffer);
			}
		}
		for (int row=0; row<nH2; row++)
		{
			...
			{
				dValue = distortion_coeff->data.fl[row * distortion_coeff->cols + col];
				sprintf(buffer, "%.4f", dValue);
				fun.WriteTxt(file, buffer);
			}
		}
	...
	else
	{
		...
		{
		    cout<<"Distortion file open failed "<<endl;
		    return;
		}
		...
		while(!out.eof())
		{
			...
			if (i<nW1*nH1)
			{
				row = i/nW1;
				col = i%nW1;
				intrinsics->data.fl[row * intrinsics->cols + col] = (float)atof(sValue);
				dValue = (double)intrinsics->data.fl[row * intrinsics->cols + col];
				cout<<dValue<<endl;
			}
			else if (i>=nW1*nH1 && i<(nH1*nW1 + nH2*nW2))
			{
				row = (i - nH1*nW1)/nW2;
				col = (i - nH1*nW1)%nW2;
				distortion_coeff->data.fl[row * distortion_coeff->cols + col] = (float)atof(sValue);
				dValue = (double)distortion_coeff->data.fl[row * distortion_coeff->cols + col];
				cout<<dValue<<endl;
			}
			i++;
		}
...
void CCameraProcess::distortionData(const char* file, Mat &intrinsics, Mat &distortion_coeff, bool isRead)
{
       // string sFilePath;
	CCommonFunClass fun;
	int nW1 = intrinsics.cols;
	...
	else
	{
		...
		fstream out;
            /*    char filename[256];  ///////////////////
                ...
                cout<<"distortion file"<<file<<endl;*/
		out.open(file, ios::in); //sFilePath
		if(out.fail())//文件打开失败:返回0
		{
			cout<<"Distortion file open failed "<<endl;
			return;
